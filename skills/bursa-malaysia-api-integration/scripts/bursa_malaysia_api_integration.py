...
class BursaConfig:
    sender_comp_id: str
    target_comp_id: str  # e.g., 'FIXTRADER' or 'FIXNEGDEAL'
    host: str
    port: int
    heartbeat_interval: int = 30
    protocol_version: str = "FIX.5.0SP1"
...
class BursaMalaysiaFixEngine:
    """
    Institutional grade FIX integration engine for Bursa Malaysia BTS2.
    Handles session management and order routing simulation over FIX 5.0 SP1.
    """
    def __init__(self, config: BursaConfig):
        ...
        if config.protocol_version != "FIX.5.0SP1":
            raise ValueError("Bursa Malaysia BTS2 requires FIX.5.0SP1")
            
        logger.info(f"Initialized BTS2 FIX Engine for {config.sender_comp_id} targeting {config.target_comp_id}")

    def connect(self) -> bool:
        """Simulates establishing a FIX session with BTS2."""
        logger.info(f"Connecting to BTS2 FIX Gateway at {self.config.host}:{self.config.port}...")
    ...
    def submit_order(self, order: FIXOrder) -> str:
        """Submits a NewOrderSingle (MsgType=D) to BTS2."""
