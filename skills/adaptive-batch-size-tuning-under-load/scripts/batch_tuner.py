...
class AdaptiveBatchTunerEngine:
    ...
    Thread-safe implementation using deque for O(1) pops and monotonic time for accurate
    interval tracking without NTP drift interference.
    """
    ...
    def __init__(
        ...
        self.ewma_write_latency_ms = 0.0
    ...
    def record_write_latency(self, latency_ms: float) -> None:
        """Records downstream sink write latency and updates EWMA to apply feedback throttling."""
        with self._lock:
            if self.ewma_write_latency_ms == 0.0:
                self.ewma_write_latency_ms = latency_ms
            else:
                self.ewma_write_latency_ms = (
                    self.latency_ewma_alpha * latency_ms 
                    + (1 - self.latency_ewma_alpha) * self.ewma_write_latency_ms
                )

            # If smoothed DB latency is high, reduce batch size to relieve DB lock contention
            if self.ewma_write_latency_ms > self.target_write_latency_ms:
                new_size = max(self.min_batch_size, int(self.current_batch_size * 0.8))
                logger.warning(
                    f"DB Latency Breach (EWMA {self.ewma_write_latency_ms:.1f}ms > {self.target_write_latency_ms:.1f}ms). "
                    f"Throttling batch size {self.current_batch_size} -> {new_size}"
                )
                self.current_batch_size = new_size

    def _extract_batch(self) -> List[Any]:
        # O(1) operations with deque vs O(N) list slicing
        count = min(len(self.queue), self.current_batch_size)
        batch = [self.queue.popleft() for _ in range(count)]
    ...
    def get_status(self, queue_capacity: int = 2000) -> BatchTunerStatus:
        with self._lock:
            ...
            return BatchTunerStatus(
                ...
                ewma_write_latency_ms=self.ewma_write_latency_ms,
