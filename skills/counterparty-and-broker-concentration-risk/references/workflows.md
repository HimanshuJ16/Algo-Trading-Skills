...
4. **Broker HHI Metric**:
   - Compute $HHI = \sum w_k^2$ (1/n for equal exposure, 1.0 for a single broker). A warning is logged when $HHI > 0.35$ (configurable `hhi_alert_threshold`) — treat the level as an engineering default, calibrated to the fund's counterparty policy.
