...
4. **Pre-Trade Firm-Wide Limit Audit** (`evaluate_pre_trade_order`):
   - For a proposed order in Sub-Account $A$: recompute projected $\text{GMV}_{firm}$ and margin utilization on the exact post-trade book (the traded symbol is valued at the live order price, not a stale feed).
...
- **Mislabeling Offsetting Positions as Wash Trades**: Regulatory wash trades are *executions* without change in beneficial ownership under common ownership (SEC Exchange Act §9(a), CEA §4c(a)); merely holding offsetting positions is not one. Conversely, actually *crossing* orders between commonly-owned sub-accounts CAN implicate those rules — run `wash-trade-and-spoofing-self-detection`.
