...
- [ ] `max_latency_ms`, `max_http_5xx_error_rate`, `max_order_reject_rate` set and within valid ranges (rates in `[0, 1]`).
...
- [ ] Second rollback within cooldown returns `SUPPRESSED`.
- [ ] `max_rollbacks_per_deployment` reached -> `SUPPRESSED` + human escalation.
