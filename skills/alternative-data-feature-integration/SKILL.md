...
- The source contains **unscrubbed PII** or cannot meet the anonymization/panel-aggregation threshold (>= 50 contributors per panel cell).
...
2. **PII / anonymization assertion**: assert PII is scrubbed and any panel/aggregated data meets the >= 50-contributor threshold (no small-cell re-identification).
...
4. **Align to Trading Timeline**: Pass a list of target naive-UTC trading times to `align_to_trading_schedule()`. It returns, per trading time and per source, an `AlignedValue` with the last-known value, its `knowledge_timestamp`, `age`, and a `staleness_state`.
...
- **Deterministic**: re-ingesting the same events in any input order yields the same `pit_features` sequence (tie-break is `(knowledge_timestamp, source_id)`).
