...
- **[planned]** `saxo-bank-openapi-integration` — Integrating Saxo Bank's OpenAPI for multi-asset European/global trading.
- **[planned]** `tastytrade-api-integration` — Integrating Tastytrade's API for options-focused US retail trading.
...
- **[planned]** `vectorized-vs-event-driven-backtest-tradeoffs` — Choosing between a fast vectorized backtest engine and a slower but more realistic event-driven engine based on strategy characteristics.
- **[planned]** `multi-year-regime-coverage-requirement` — Requiring backtest data to span multiple distinct market regimes (bull, bear, high-vol, low-vol) before trusting reported performance.
...
- **[planned]** `backtest-parameter-sensitivity-analysis` — Testing how sensitive a strategy's backtested performance is to small parameter changes, to detect an overfit 'sweet spot' versus a genuinely robust setting.
...
- **[planned]** `backtesting-ml-models-against-transaction-costs` — Ensuring ML model backtests apply the same realistic transaction-cost modeling as rule-based strategies (see execution-realistic-simulation), since ML signals are just as vulnerable to cost-blind overstatement.
- **[planned]** `categorical-feature-encoding-for-instrument-identity` — Encoding instrument identity (symbol, sector) as a categorical feature without leaking future-only classification info (e.g. sector reclassifications).
