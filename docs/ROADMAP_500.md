...
- **[BUILT]** `schwab-api-oauth-pkce-flow` — Implementing Schwab's (post-TD-Ameritrade-migration) OAuth2 authorization-code flow — which is confidential-client, not PKCE — and living with its hard 7-day refresh-token window.
...
## real-time-architecture  _(31 tracked: 31 built, 0 planned)_
...
## backtesting-methodology  _(31 tracked: 31 built, 0 planned)_
...
- **[BUILT]** `risk-adjusted-performance-attribution-per-strategy` — Attributing risk-adjusted performance (not just raw P&L) per strategy when multiple strategies share a risk budget.
